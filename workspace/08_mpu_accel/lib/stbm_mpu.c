...
uint8_t mpu_set_bypass(uint8_t bypass_on){
	...
	if (mpu9250_st.bypass_mode == bypass_on)
		return 0;

	if (bypass_on) {
		if (mpu9250_reads(MPU6500_USER_CTRL, 1, &tmp))
			return -1;
		tmp &= ~BIT_AUX_IF_EN;
		if (mpu9250_writes(MPU6500_USER_CTRL, 1, &tmp))
			return -1;
		delay_ms(3);
		tmp = BIT_BYPASS_EN;
		if (mpu9250_st.active_low_int)
			tmp |= BIT_ACTL;
		if (mpu9250_st.latched_int)
			tmp |= BIT_LATCH_EN | BIT_ANY_RD_CLR;
		if (mpu9250_writes(MPU6500_INT_PIN_CFG, 1, &tmp))
			return -1;
	} else {
		/* Enable I2C master mode if compass is being used. */
		if (mpu9250_reads(MPU6500_USER_CTRL, 1, &tmp))
			return -1;
		if (mpu9250_st.sensors & MPU_XYZ_COMPASS)
			tmp |= BIT_AUX_IF_EN;
		else
			tmp &= ~BIT_AUX_IF_EN;
		if (mpu9250_writes(MPU6500_USER_CTRL, 1, &tmp))
	...
	mpu9250_st.bypass_mode = bypass_on;
	return 0;
}
...
uint8_t mpu_set_int_latched(uint8_t enable){
	uint8_t tmp;
	if (mpu9250_st.latched_int == enable)
		return 0;
	...
	if (enable)
		...
		tmp = 0;
	if (mpu9250_st.bypass_mode)
		tmp |= BIT_BYPASS_EN;
	if (mpu9250_st.active_low_int)
		tmp |= BIT_ACTL;
	if (mpu9250_writes(MPU6500_INT_PIN_CFG, 1, &tmp))
		return -1;
	mpu9250_st.latched_int = enable;
	return 0;
}
...
uint8_t mpu_get_accel_reg(int16_t *data){
	...
	//if (!(mpu9250_st.sensors & MPU_XYZ_ACCEL))
	//	return -1;
	
	if (mpu9250_reads(MPU6500_RAW_ACCEL, 6, tmp))
		return -1;
...
uint8_t mpu_get_gyro_reg(int16_t *data){
	...
	if (mpu9250_reads(MPU6500_RAW_GYRO, 6, tmp))
		return -1;
...
uint8_t mpu_who_am_i(uint8_t *data){
	if (mpu9250_reads(MPU6500_WHO_AM_I, 1, data))
		return -1;
